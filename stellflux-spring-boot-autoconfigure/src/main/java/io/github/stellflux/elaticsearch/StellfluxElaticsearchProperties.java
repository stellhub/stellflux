...
/** Stellflux Elaticsearch 自动装配配置。 */
@ConfigurationProperties(prefix = "stellflux.elaticsearch")
public class StellfluxElaticsearchProperties {
     ...
     * 转换为核心模块配置。
     *
     * @return Elaticsearch 客户端配置
     */
    public StellfluxElaticsearchOptions toOptions() {
        StellfluxElaticsearchOptions options = new StellfluxElaticsearchOptions();
        options.setEndpoints(endpoints);
        options.setUsername(username);
        options.setPassword(password);
        options.setApiKey(apiKey);
        options.setPathPrefix(pathPrefix);
        options.setConnectTimeoutMillis(connectTimeoutMillis);
        options.setSocketTimeoutMillis(socketTimeoutMillis);
        options.setConnectionRequestTimeoutMillis(connectionRequestTimeoutMillis);
        options.setCompressionEnabled(compressionEnabled);
        options.setMetaHeaderEnabled(metaHeaderEnabled);
        return options;
    }
}
