...
public class StellfluxGrpcServerTelemetryInterceptor implements ServerInterceptor {
    ...
    public <ReqT, RespT> ServerCall.Listener<ReqT> interceptCall(
        ...
        ServerCall<ReqT, RespT> telemetryCall =
                new ForwardingServerCall.SimpleForwardingServerCall<>(call) {
                    ...
                    public void close(Status status, Metadata trailers) {
                        try {
                            finalizeCall(method, span, startNanos, status, null, ended);
                        } finally {
        ...
        try (Scope ignored = context.makeCurrent()) {
            ...
            finalizeCall(method, span, startNanos, Status.UNKNOWN, exception, ended);
            throw exception;
        }
        ...
        return new ForwardingServerCallListener.SimpleForwardingServerCallListener<>(delegate) {
            ...
            public void onCancel() {
                try (Scope ignored = context.makeCurrent()) {
                    finalizeCall(method, span, startNanos, Status.CANCELLED, null, ended);
                    super.onCancel();
                }
            }
            ...
            public void onComplete() {
                try (Scope ignored = context.makeCurrent()) {
                    finalizeCall(method, span, startNanos, Status.OK, null, ended);
                    super.onComplete();
                }
            }
